...
#função para download e extração dos datasets anteriores
...
```{r prepoc1}
# Criar um objeto SingleCellExperiment
sce <- SingleCellExperiment(assays = list(counts = raw_counts))

# Amostragem de células para reduzir o tamanho do dataset
set.seed(123)
sample_cells <- sample(colnames(sce), size = 10000)

# 'sce_sample' é um objeto SingleCellExperiment e deve conter apenas as células da amostra
sce_sample <- SingleCellExperiment(assays = list(counts = raw_counts[, sample_cells]))

# Clusterização rápida com a amostra
clusters <- quickCluster(sce_sample)
# Convert clusters to numeric if it is a factor
if (is.factor(clusters)) {
    clusters <- as.numeric(as.character(clusters))
}

# Computar os fatores de tamanho utilizando apenas a amostra
size_factors <- computeSumFactors(sce_sample, clusters = clusters)

# Computar os fatores de tamanho utilizando apenas a amostra
size_factors <- computeSumFactors(sce_sample, clusters = clusters)

# Verifique novamente se o tamanho do vetor size_factors corresponde ao número de células em sce_sample
if (length(size_factors) == length(sample_cells)) {
  # Armazenar os fatores de tamanho no objeto SingleCellExperiment da amostra
  sizeFactors(sce_sample) <- size_factors
  # Se não houver erro, a função continuará normalmente
} else {
  # Se o número de fatores de tamanho não corresponder, um erro será lançado
  stop("O número de fatores de tamanho não corresponde ao número de células na amostra.")
}
